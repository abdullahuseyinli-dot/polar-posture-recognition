...
def comparisons(summary: dict) -> None:
    ...
    for ax, task, title in zip(
        ...
        for y, name in enumerate(names):
            ...
            ax.text(99.6, y, f"{value:.2f}", ha="right", va="center", color=color, fontsize=10)
    ...
    fig.text(
        0.05,
        0.038,
        "Bars: marginal 95% source-group bootstrap intervals. Promotion uses paired tests and all locked gates.",
        fontsize=10,
        color=GREY,
    )
...
def confusion(summary: dict) -> None:
    ...
    ax.set_xlabel("Predicted posture", labelpad=12)
    ax.set_ylabel("Annotated posture", labelpad=12)
    ax.tick_params(length=0)
    fig.colorbar(chart, ax=ax, fraction=0.035, pad=0.03, label="Row share (%)")
